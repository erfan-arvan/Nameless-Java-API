...
public class RequestHandler {
    ...
    RequestHandler(final URL apiUrl, final Methanol httpClient, final Gson gson, final ApiLogger debugLogger, final int responseLengthLimit) {
    ...
    public JsonObject post(final String route, final JsonObject postData) throws NamelessException {
        return makeConnection(route, postData);
    }

    public JsonObject get(final String route, final Object... parameters) throws NamelessException {
    ...
    private void debug(final String message, final Supplier<Object[]> argsSupplier) {
        if (this.debugLogger != null) {
            this.debugLogger.log(String.format(message, argsSupplier.get()));
        }
    }

    private JsonObject makeConnection(final String route, final JsonObject postBody) throws NamelessException {
        ...
        debug("Making connection %s to %s", () -> new Object[] { postBody != null ? "POST" : "GET", request.uri() });
        if (postBody != null) {
        ...
        try {
            ...
            if (exceptionMessage != null && exceptionMessage.contains("unable to find valid certification path to requested target")) {
        ...
        try {
            ...
            message.append("-----------------\n");
            // fit in a Discord message with safety margin
